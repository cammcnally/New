...
> Current file: `docs/assessments/pipeline-auditor-latest.md`

# Assessment Metadata

- Assessment Type: pipeline_auditor_assessment
- Status: SUPERSEDED
- Assessed At: 2025-03-10 00:00 AEDT
- Assessed From Commit: not-recorded-pre-lifecycle
- Assessed From Branch: unknown-pre-lifecycle
- Scope: Historical pipeline auditor report captured before lifecycle hardening
- Supersedes: none
- Superseded By: docs/assessments/pipeline-auditor-latest.md
...
**Scope:** Full repository â€” Pipeline.py (line-by-line), config, data flows, Blueprint specs  
...
- **Outer-boundary purge** on `event_end_time` with assertion (L3037â€“3040)
- **Threshold holdout** scored out-of-sample; thresholds selected only on that holdout (L3305)
- **Calibration holdout** chronologically later than meta-fit; calibrator fit on OOS data (L2061â€“2097)
- **Imputation** fit on training data only (`impute_fit_transform` L1792â€“1796)
...
**Config / data:**
- `PipelineConfig` dataclass (L65â€“137)
...
| 11 | Threshold split | 3163â€“3182 | `threshold_fit_df` vs `threshold_holdout_df` |
...
| 16 | Subset search | 3266â€“3332 | On threshold_fit vs threshold_holdout |
| 17 | Permutation importance | 3351â€“3386 | Train fit, test permute |
| 18 | Regime IC | 3388â€“3420 | high_vol_34, low_vol_34 (vol_cluster_89 optional) |
| 19 | Seed robustness | 3787â€“3892 | `evaluate_seed_robustness()` when seed_mode != single |
...
- **Calibrator:** fit on `(logit(raw_calib_meta), y_calib)` where `raw_calib_meta` = meta predictions on base predictions on `calibration_holdout_df` (L2092â€“2097)
...
- **XS z-scores:** `groupby("timestamp_utc")` â€” at each t, mean/std over tickers at t only (L1430â€“1434)
...
| Base models | meta_fit_df | calibration_holdout, test | L2082â€“2085, L2067â€“2068 |
| Meta model | OOF from meta_fit_df | calibration_holdout, test | L2086â€“2097 |
| Calibrator | calibration_holdout_df | test | L2097 |
| Threshold selection | â€” | threshold_holdout_scored only | L3305 |
| Subset search | threshold_fit_df | threshold_holdout_df | L3292â€“3310 |
| Permutation importance | train_df | test_df | L3351â€“3386 |
...
- **Severity:** Low
- **Confidence:** High
- **Evidence:** L1490â€“1492: `if hit_stop and hit_target: outcome = 0`. Internally consistent but undocumented.

### 6.3 Design tradeoffs

**[DT1] Spearman IC: pooled vs timestamp-level**

- **Evidence:** `spearman_ic()` (L563) uses pooled `s.corr(o, method="spearman")`. Blueprint recommends timestamp-level cross-sectional Spearman. `spearman_ic_by_timestamp()` (L419) exists and is used for fold metrics (L3347â€“3353); pooled `spearman_ic` is also used for binary IC.
...
- **Evidence:** L3308â€“3314: `subset_score = 0.35*pr_auc + 0.25*roc_auc - 0.25*log_loss - 0.15*brier`. Blueprint prefers portfolio metrics for final promotion. Comment at L3306â€“3307 explains proxy.
...
- **Evidence:** L3393â€“3396 check for these columns; `add_per_ticker_features` (L1375â€“1376) only creates `vol_cluster_high_34`, `vol_cluster_low_34`. VOLATILITY_CLUSTERING_FEATURES (L203â€“224) does not list _89 variants.
...
| Calibrator on meta training data | Calibrator fit on `calibration_holdout_df` L2092â€“2097 | **Not confirmed** |
...
| Subset search on test | Uses threshold_fit_df vs threshold_holdout_df L3292â€“3293 | **Not confirmed** |
| Permutation on train labels | perm_model fit on train, permute test features L3351â€“3386 | **Not confirmed** |
...
- **Model seeds:** RF/ET/XGB/LGBM/ENET use `config.random_seed + offset` (L1604â€“1666)
...
- **Threshold selection:** `choose_thresholds(threshold_holdout_scored, ...)` â€” grid over p_min, theta_ev, theta_rel; `research_score()` for ranking
...
| DT1 Timestamp-level IC | â€” | Already implemented; used in fold metrics |
